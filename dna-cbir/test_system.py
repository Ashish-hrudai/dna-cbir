"""
Demo script to test DNA-CBIR system
"""
import numpy as np
import cv2
from dna_encoding import DNAEncoder, DNATranslator, SimilarityMatcher
from cbir_models import InstanceBasedCBIR, compute_retrieval_metrics
import os
...
def create_sample_images():
    ...
    for i in range(3):
        ...
        cv2.imwrite(f'database/pattern_{i}.png', cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        images.append(('pattern', f'pattern_{i}.png', img))
...
def test_instance_based_cbir():
    """Test instance-based CBIR (Model I)"""
    print("\n" + "="*60)
    print("TESTING INSTANCE-BASED CBIR (MODEL I)")
    print("="*60)
    ...
    encoder = DNAEncoder()
    matcher = SimilarityMatcher()
    cbir = InstanceBasedCBIR(encoder, matcher, use_multi_scale=False)
    
    # Build feature space
    print("\n1. Building feature space...")
    db_images = [img for _, _, img in images]
    db_paths = [path for _, path, _ in images]
    cbir.build_feature_space(db_images, db_paths)
    print(f"   ✓ Built feature space with {len(db_images)} images")
    
    # Test retrieval with a red image
    print("\n2. Testing retrieval with RED query image...")
    query_img = images[0][2]  # First red image
    results = cbir.retrieve_similar_images(query_img, k=5, metric='weighted')
    
    print("   Top 5 Results:")
    for idx, (img_idx, score) in enumerate(results, 1):
        print(f"   {idx}. {db_paths[img_idx]} - Score: {score:.4f}")
    
    # Test with multi-scale
    print("\n3. Testing with multi-scale features (IDEA 2)...")
    cbir_multi = InstanceBasedCBIR(encoder, matcher, use_multi_scale=True)
    cbir_multi.build_feature_space(db_images, db_paths)
    results_multi = cbir_multi.retrieve_similar_images(query_img, k=5, metric='weighted')
    
    print("   Top 5 Results (Multi-Scale):")
    for idx, (img_idx, score) in enumerate(results_multi, 1):
        print(f"   {idx}. {db_paths[img_idx]} - Score: {score:.4f}")
    
    # Compare different metrics
    print("\n4. Comparing different metrics (IDEA 3)...")
    metrics = ['euclidean', 'cosine', 'chi_square', 'weighted']
    for metric in metrics:
        results = cbir.retrieve_similar_images(query_img, k=3, metric=metric)
        top_result = db_paths[results[0][0]]
        print(f"   {metric:12s}: Top result = {top_result}")
    
    print("\n✓ Instance-based CBIR working correctly")
    return True

def test_dna_translation():
    """Test DNA translation for Model II"""
    print("\n" + "="*60)
    print("TESTING DNA TRANSLATION (FOR MODEL II)")
    print("="*60)
    ...
    print(f"   ✓ Amplification factor: {amplified.max() / numeric.max():.1f}x")
...
def run_all_tests():
    ...
    tests = [
        ...
        ("Instance-Based CBIR", test_instance_based_cbir),
